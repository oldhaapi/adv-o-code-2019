  ...
  (loop [curpos [0 0]
    ...
    (if (empty? remain)
      ...
      (let [[wvs & remaining] remain
        ...
        (recur b-pt
               remaining
               (into segments [a-pt b-pt]))))))

(defn taxi-distance
  "Compute taxi distance of point from origin"
  [[x-in y-in]]
  (let [x (if (neg? x-in) (- x-in) x-in)
        y (if (neg? y-in) (- y-in) y-in)]
    (+ x y)))

(defn dist-to-x-point
  "Find the distance to the given intersection point on a wire
...
(defn -main
  "Given the segment data from two wires' routes on a grid,
  return the Manhattan distance from the closest intersection
  point to the origin"
  [& args]
  (let [warg1 (if (empty? args) wire1-input (first args))
        warg2 (if (empty? args) wire2-input (second args))
    ...
    ;;   (println "wire2 pts:" wire2-points))

    (println "count of x-points:" (count x-points))
    ; Skip [0 0]
    (let [closest (second (sort (map #(vector (taxi-distance %) %) x-points)))
          [clsdist [xc yc]] closest]
      (println "Closest intersection is at point" xc yc "at distance" clsdist))
