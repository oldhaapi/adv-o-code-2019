  ...
  (let [ip (atom 0)]
    (fn [tape]
      ...
      (let [code (aget tape @ip)
            ...
            immbits (imm code)
            _ (print (format "%3s %04d/%d" @ip code oplen) opkw params) ]
...
(defn doop
  ...
  (let [opkw (opint-to-opkw code)
        val (cond
              ;; HLT
              (= opkw :HLT) nil
              ;; PRN or INP, no immediate mode
              (or (= opkw :PRN) (= opkw :INP) (= opkw :RBS))
              (let [posval (first params)
                    pos (if (rel? (first immbits)) (+ @relbase posval) posval)]
                (f tape pos))
              ;; No two-param ops
              ...
              (let [p1 (iorp tape (first immbits) (first params))
                    ...
                    posval (first (nthrest params 2))
                    pos (if (rel? (first (nthrest immbits 2))) (+ @relbase posval) posval)]
...
(defn intcode [src-tape inp-seq]
  (let [input-func (in-func inp-seq)
        ...
        tape (long-array 1000000 src-tape)
        _ (println "Program size is:" tapelen)]
    ;; move the src-tape longs into the tape
